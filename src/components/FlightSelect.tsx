...
function FlightSelect({
  ...
  const { fromLocation, setFromLocation, toLocation, setToLocation } = useLocations();
  ...
  useEffect(() => {
    if (type === 'from') {
      getUserLocation().then((location) => {
        flightsApi.getNearByAirports<NearByApiResponse>(location).then((response) => {
          ...
          if (resultResponse.status) {
            ...
            if (locationData.current) {
              setValue({
                skyId: locationData.current.navigation.relevantFlightParams.skyId || undefined,
                entityId:
                  locationData.current.navigation.relevantFlightParams.entityId || undefined,
              });
    ...
    label: (
      <div className="flex content-center justify-start py-2">
        ...
        <div className="flex flex-col">
          <span className="font-medium text-gray-800">{result.presentation.suggestionTitle}</span>
          <span className="text-sm text-gray-500">{result.presentation.subtitle}</span>
        </div>
      </div>
    ),
...
export default memo(FlightSelect);
